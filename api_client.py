...
#    url = self.base_url + endpoint
#    response = request.get(url, headers=self.headers, json=data)
#    if status.code:
#        assert response.status_code == status_code
#    return response.json()
...
#            url = f"{self.base_url}{PING_ENDPOINT.value}"
#            response = self.session.get(url)
#            response.raise_for_status()
#        with allure.steps('Assert status code'):
#            assert response.status_code == 201, f"Expected status 201 but got {response.status_code}"
#        return response.status_code

#    def auth(self):
#        with allure.step('Getting authenticate'):
#            url = f"{self.base_url}{AUTH_ENDPOINT.value}"
#             payload = {"username": Users.USER_NAME.value, "password": Users.Password.value}
...
#        with allure.steps('Updating header with authorization'):
#            self.session.headers.update({"Authorization": f"Bearer" {token}"})

#    def get_booking_by_id(self, booking_id):
#       with allure.step('Getting object with booking id'):
#        url = f"{self.base_url}{Endpoints.BOOKING_ID_ENDPOINT.value}/{booking_id}"
#        response = self.session.get (url)
#        response.raise_for_status ()
#       with allure.steps('Checking status code'):
#        assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
